...
- Tool-Calling Agent (ReAct loop) вЂ” the model searches products and manages a shopping cart by calling tools in an iterative ReAct loop. You will write docstrings for the tool functions, generate a tool schema with convert_to_openai_tool, and implement run_shopping_agent вЂ” the ReAct loop that sends messages to the LLM, executes tool calls, and returns the final text response.
- Memory Agent вЂ” the agent gains long-term memory (a user profile saved to disk) and short-term memory (conversation history), so it remembers preferences across sessions and past search results within a session. You will implement load_profile / save_profile for reading and writing a JSON profile, define the update_profile tool schema, and implement run_memory_agent that injects the profile into the system prompt and carries conversation history between turns.
- Multi-Agent System вЂ” four specialized agents вЂ” Retriever, Pros Analyst, Cons Analyst, and Ranker вЂ” coordinated by an Orchestrator that delegates work, collects results, and produces a final recommendation with honest pros & cons. You will implement all four agent classes and the CoordinatorAgent that runs them in a chain, passing data through a shared AgentContext.

Detailed instructions, starter code, and open test cases are in the notebook.

Important: all submissions are graded using the gpt-oss-20b model. Use the same model when developing and testing your solutions вЂ” this way the agent's behavior during grading will match what you observe locally.
...
- ✔️ Paraphrase a text while preserving its meaning
- Count how many times the letter вЂњrвЂќ appears in the word strawberry
- ✔️ Generate a short summary of a long article
- Report the current EUR exchange rate
- ✔️ Explain the difference between two programming concepts
...
- ✔️ The LLM may start treating tool outputs as normal conversation text and become less reliable at deciding when to call tools
...
In an agent system, an LLM has access to a tool get_weather(city). A user asks: вЂњWhat is the weather in Berlin right now?вЂќ How does the tool call actually occur?

**Choose correct option**

- The LLM directly executes the get_weather function in the runtime and receives the result
- ✔️ The LLM outputs a structured tool call; the agent executes the tool and sends the result back to the model
...
- charges the userвЂ™s payment method
...
- ✔️ The tool should check whether the booking or payment already exists before executing again
...
- ✔️ A host creates MCP clients to connect to MCP servers
- ✔️ MCP servers execute tools and return structured results
- ✔️ A single host can connect to multiple MCP servers using clients
...
- ✔️ Continued Pretraining = knowledge; SFT = behavior
...
- Always use Training вЂ” it gives better results
- Use RAG for factual questions and Training for creative tasks
- ✔️ RAG first; Training when RAG hits its ceiling
...
- ✔️ Injection overrides agent instructions; Jailbreaking overrides model safety
...
- Nowhere вЂ” ReAct is always better
- ✔️ Arithmetic tasks
- Question answering
- Tasks with tools
...
- ✔️ Both factors + attention degradation on long tool descriptions
- This is a myth вЂ” more tools is always better
...
- Speed вЂ” too slow
- ✔️ How to achieve consensus / avoid infinite communication loops
...
You run your LLM-as-Judge on 50 test cases and get Cohen's Оє = 0.2 for the "efficiency" dimension. What is the right next step?

**Choose correct option**

- Increase the number of test cases to 500 вЂ” Оє is too noisy on small samples
- ✔️ Look at the specific cases where judge and human disagree, find a pattern, fix the prompt
- Switch to a stronger model вЂ” GPT-4 will give higher Оє automatically
- Accept the result вЂ” Оє = 0.2 is above zero, the judge is working
...
- 100 runs is too few вЂ” you need at least 1000
- ✔️ The test checks the agent's text, not the actual state of the booking in the database
- The agent should be tested by humans, not automatically
- There is no problem вЂ” 87% is a good result
...
Your team spent two weeks writing a detailed, carefully calibrated prompt for a GPT-4o mini judge вЂ” and achieved Оє = 0.81. Management asks you to cut costs and switch to GPT-3.5. What will most likely happen?

**Choose correct option**

- Оє will stay the same вЂ” the prompt is the same, only the model changes
- Оє will improve вЂ” a simpler model follows instructions more literally
- ✔️ Оє may drop significantly вЂ” the prompt was co-tuned with the stronger model and may not transfer
- Оє will drop slightly, within measurement error

## Task X

### Q21 Chain-of-thought reasoning

A team wants to find out whether adding chain-of-thought reasoning to their judge prompt improves agreement with human labels. In the same experiment, they also switch from GPT-4o mini to GPT-4o. After the experiment, Оє goes up from 0.41 to 0.67. What conclusion can they draw?

**Choose correct option**

- Chain-of-thought improved the judge
- The stronger model improved the judge
- ✔️ Both changes together improved the judge, but it is impossible to say which one caused the gain
- The result is unreliable вЂ” both changes must always be tested together
...
- ✔️ Semantic tool filtering. Give the agent only 3вЂ“5 relevant tools per request
...
- ✔️ Break task into multi-step pipeline with fast routing + slow reasoning only when needed
...
- Add a retry loop
- ✔️ Move refund decision logic to deterministic rules and use LLM only for interpretation
...
- ✔️ Data in RAG became outdated (knowledge rot)
